...
void BlocksDetector::Start()
{
	if (1 == 1)//用IF隔离局部代码
	{
		...
		if (OUTPUT_DEBUG_INFO)
		{
			...
			cv::line(drowDebugResult, cv::Point(((*mdi).MaxPics - LeftLine.y0) / LeftLine.k + LeftLine.x0, (*mdi).MaxPics), cv::Point((0 - LeftLine.y0) / LeftLine.k + LeftLine.x0, 0), cv::Scalar(180, 100, 255), 1);
	...
	if (2 == 2)
	{
		...
		if (OUTPUT_DEBUG_INFO)
		{
			...
			cv::line(drowDebugResult, cv::Point(((*mdi).MaxPics - RightLine.y0) / RightLine.k + RightLine.x0, (*mdi).MaxPics), cv::Point((0 - RightLine.y0) / RightLine.k + RightLine.x0, 0), cv::Scalar(180, 100, 255), 1);
...
void BlocksDetector::StartUP_DOWN(BorderType bt)
{
	...
	if (OUTPUT_DEBUG_INFO)
	{
		...
		cv::line(drowDebugResult, cv::Point(0, (*line).k * (0 - (*line).x0) + (*line).y0), cv::Point((*mdi).width, (*line).k * ((*mdi).width - (*line).x0) + (*line).y0), cv::Scalar(180, 255, 100), 1);
