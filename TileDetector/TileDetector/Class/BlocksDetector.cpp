...
void BlocksDetector::Start()
{
	if (1 == 1)
	{
		...
		if (OUTPUT_DEBUG_INFO)
		{
			...
			cv::line(drowDebugResult, cv::Point(((*mdi).MaxPics - y0) / k + x0, (*mdi).MaxPics), cv::Point((0 - y0) / k + x0, 0), cv::Scalar(255, 255, 255), 1);
