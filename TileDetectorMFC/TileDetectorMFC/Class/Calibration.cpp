...
bool SortBysize1(const vector<Point>v1, const vector<Point>v2)//注意：本函数的参数的类型一定要与vector中元素的类型一致  
{
	return v1.size() > v2.size();//降序排列  
}

bool Calibration::WhetherLine(Mat &oneImg, Mat &G_Img, bool cor, bool boe)//判断是否为line的核心部分
{
...
int Calibration::ConsumeItem(ItemRepository *ir)
{
	...
	for (size_t i = 0; i < CneedContours.size(); i++)
	{
		...
		Mat growImg = C_Grow(ready_boxImg, centermark, input_th_v);//区域生长
		...
		minMaxLoc(box_histolist, NULL, NULL, NULL, &modePoint);
		int box_h = modePoint.y + 5;
		int box_h_sum = 0;
		for (int j = box_h; j < max_v + 1; j++)
		{
			box_h_sum += box_histolist.at<float>(j);
		}
		int box_l_sum = box_histolist.at<float>(min_v)+box_histolist.at<float>(min_v + 1) + box_histolist.at<float>(min_v + 2) + box_histolist.at<float>(min_v + 3);
		if (min_v > 35 && max_v < avgmean[0] + 5)
			continue;
		if (max_v - min_v < 25)
			continue;
		if (min_v <= 35)
		...
		Scalar mount = mean(ano_boxImg, growImg);
...
void Calibration::ProducerTask() // 生产者任务
{
...
void Calibration::ConsumerTask() // 消费者任务
{
	static int cnt = 1;
	while (1) {
...
void Calibration::linedetect()
{
	...
	for (size_t i = 0; i < bigcontours.size(); i++)
	{
		...
		threshold(fingerprint_Img, fingerprint_Img, CannyImg.at<uchar>(0.5*(bigcontours[i][0] + bigcontours[i][bigcontours[i].size() - 1])), 255, 1);
		if (countNonZero(fingerprint_Img) >= 0.3 * fingerprint_Img.cols * fingerprint_Img.rows)
			continue;
		vector<vector<Point>> fingerprintcontours;
		rectangle(fingerprint_Img, Rect(Point(1, 1), Point(fingerprint_Img.cols - 1, fingerprint_Img.rows - 1)), Scalar(255), -1);
		findContours(fingerprint_Img, fingerprintcontours, RETR_EXTERNAL, CV_CHAIN_APPROX_SIMPLE);
		if (fingerprintcontours.size() != 1)
			continue;
...
void Calibration::tem_2plate()//模板轮廓匹配预处理线程
{
...
void Calibration::img2clone()//待处理大图复制线程
{
...
void Calibration::img2zoom()//划痕检测缩放预处理
{
...
void Calibration::C_pretreatment(Mat &image, Block *_block, Scales *_scales)
{
	...
	std::thread producer(std::mem_fn(&Calibration::ProducerTask), this); // 待检测缺陷的预处理.
	std::thread consumer(std::mem_fn(&Calibration::ConsumerTask), this); // 区分缺陷与水渍.
	std::thread line(std::mem_fn(&Calibration::linedetect), this);//划痕检测 
